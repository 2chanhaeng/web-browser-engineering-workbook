...
def chunk_bytes(data: bytes, chunk_size: int = 16) -> Generator[bytes, None]:
...
def cache_endpoint(mode: Optional[str] = None) -> PlainTextResponse:
    ...
    response.headers["Cache-Control"] = (
        cache_value  # Controls how the browser caches this resource.
    )
    response.headers["Connection"] = (
        "close"  # Ensure the server closes the connection after the response.
    )
    return response
